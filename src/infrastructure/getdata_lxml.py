...
class GetHoldPlacePast(CommonMethods4Official):
    ...
    def holdplace2cdlist(self) -> list:
        ...
        # コードへ返還
        code_name_list = list(
            map(lambda x: jyo_master.at[x, "jyo_cd"], self.place_name_list)
        )
    ...
    def holdinfo2dict(self, hp_name: str) -> dict:
        ...
        content_dict = {"shinko": shinko, "ed_race_no": ed_race_no}
        return content_dict
...
class OfficialProgram(CommonMethods4Official):
    def __init__(self, race_no: int, jyo_code: int, date: int) -> None:
        """
        競艇公式サイトの番組表からのデータ取得
    ...
    def getplayerinfo2dict(self, waku: int) -> dict:
        ...
        # 年齢、体重
        age_weight_xpath = "/".join(
            [target_tbody_xpath, "tr[1]/td[3]/div[3]/text()[2]"]
        )
        ...
        raw_num_F, raw_num_L, raw_avg_ST = self._box_to_three_element(
            target_tbody_xpath, column_no=4
        )
...
class OfficialChokuzen(CommonMethods4Official):
    ...
    def getcommoninfo2dict(self) -> dict:
        ...
        content_dict = super()._getweatherinfo2dict(
            lx_content=self.__lx_content, table_xpath=table_xpath
        )
...
class OfficialResults(CommonMethods4Official):
    def __init__(self, race_no: int, jyo_code: int, date: int):
        """
        競艇公式サイトの結果からのデータ取得
        レース番，場コード，日付を入力し公式サイトへアクセス
...
class OfficialOdds(CommonMethods4Official):
    ...
    def _rentan_common(self, rank: int) -> dict:
        ...
        self.logger.debug(f"called {sys._getframe().f_code.co_name}.")
        # 共通メソッドを使える
        odds_hol_list = self._tanfuku_common(rank, "rentan")
    ...
    def _renfuku_common(self, rank: int) -> dict:
        """
        2/3連複オッズを抜き出し辞書型で返す
        1=2=3のオッズは return_dict[1][2][3]に格納される
        """
        ...
        odds_rv = self._tanfuku_common(rank, "renfuku")
        odds_rv.reverse()
        # 辞書で格納する
        content_rv_dict = {}
