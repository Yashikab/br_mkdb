...
class Common:
    ...
    def check_connection(cls):
        logger = getLogger(MODULE_LOG_NAME).getChild(cls.__class__.__name__)
        ...
        for i in range(10):
            try:
                ...
                logger.debug(e)
                if i == 9:
                    logger.error("mysql connecting comfirmation: timeouted")
                else:
...
class CloudSqlController(DatabaseController):
    ...
    def clean(self):
        ...
        pipe = subprocess.Popen(cmd1, stdout=subprocess.PIPE)
        subprocess.run(cmd2, stdin=pipe.stdout)
