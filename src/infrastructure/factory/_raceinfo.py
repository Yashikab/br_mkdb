...
class RaceInfoFactoryImpl(RaceInfoFactory):
    ...
    def getinfo(self, target_date: date) -> Iterator[HoldRaceInfo]:
        ...
        target_url = "?".join([self.__base_url,
        ...
        place_names = \
            list(map(self._getplacename,
                     __lx_content.xpath(place_name_xpath)))
        ...
        shinkos = \
            list(map(self._getshinkoinfo,
                     __lx_content.xpath(shinko_info_xpath)))
    ...
    def _conversion_to_codes(self, place_names: List[str]) -> int:
        ...
        filepath = Path(__file__).resolve().parents[2].joinpath(
            "domain", "jyo_master.csv")
        ...
        code_names = \
            list(map(
                lambda x: jyo_master.at[x, 'jyo_cd'], place_names))
        return code_names
