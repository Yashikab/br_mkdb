...
from domain.model.info import ProgramInfo, ProgramPlayerInfo
...
class ProgramInfoFactoryImpl(ProgramInfoFactory):
    ...
    def _playerinfo(self, lx_content: lxml.HtmlElement) -> Iterator[ProgramPlayerInfo]:
        for waku in range(1, 7):
            ...
            player_id = self.__common.rmletter2int(raw_player_id)
            ...
            age = self.__common.rmletter2int(raw_age)
            weight = self.__common.rmletter2float(raw_weight)
            ...
            num_F = self.__common.rmletter2int(raw_num_F.strip())
            num_L = self.__common.rmletter2int(raw_num_L.strip())
            avg_ST = self.__common.rmletter2float(raw_avg_ST.strip())

            # 全国勝率
            raw_all_1rate, raw_all_2rate, raw_all_3rate = \
                self._box_to_three_element(
                    lx_content, target_tbody_xpath, column_no=5)
            all_1rate = self.__common.rmletter2float(raw_all_1rate.strip())
            all_2rate = self.__common.rmletter2float(raw_all_2rate.strip())
            all_3rate = self.__common.rmletter2float(raw_all_3rate.strip())

            # 当地勝率
            raw_local_1rate, raw_local_2rate, raw_local_3rate = \
                self._box_to_three_element(
                    lx_content, target_tbody_xpath, column_no=6)
            local_1rate = self.__common.rmletter2float(raw_local_1rate.strip())
            local_2rate = self.__common.rmletter2float(raw_local_2rate.strip())
            local_3rate = self.__common.rmletter2float(raw_local_3rate.strip())

            # モーター情報は7番目
            raw_motor_no, raw_motor_2rate, raw_motor_3rate = \
                self._box_to_three_element(
                    lx_content, target_tbody_xpath, column_no=7)
            motor_no = self.__common._rmletter2int(raw_motor_no.strip())
            motor_2rate = self.__common._rmletter2float(
                raw_motor_2rate.strip())
            motor_3rate = self.__common._rmletter2float(
            ...
            boat_no = self.__common._rmletter2int(raw_boat_no.strip())
            boat_2rate = self.__common._rmletter2float(raw_boat_2rate.strip())
            boat_3rate = self.__common._rmletter2float(raw_boat_3rate.strip())
