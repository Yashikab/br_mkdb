...
from domain.model.info import (OddsInfo, Tansho, ThreeRenfuku, ThreeRentan,
                               TwoRentan)
...
class OddsInfoFactoryImpl(OddsInfoFactory):
    ...
    def _two_rentan(self,
                    target_date: date,
                    jyo_cd: int,
                    race_no: int) -> TwoRentan:
        return self._rentan_common(
            target_date,
            jyo_cd,
            race_no,
    ...
    def _rentan_common(self,
                       target_date: date,
                       jyo_cd: int,
                       race_no: int,
                       rank: int) -> Union[Tansho, TwoRentan, ThreeRentan]:
        """
        2/3連単オッズを抜き出し辞書型で返す
