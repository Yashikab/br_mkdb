...
class TestGetHoldPlace(CommonMethod):
    ...
    @pytest.mark.parametrize(
        "date, expected", [(20200408, name_list1), (20110311, name_list2)]
    )
    def test_holdplace2strlist(self, date, expected, mocker):
        ghp = self._ghp(date, mocker)
        assert ghp.holdplace2strlist() == expected

    @pytest.mark.parametrize("date, expected", [(20110311, code_list2)])
    def test_holdplace2cdlist(self, date, expected, mocker):
        ghp = self._ghp(date, mocker)
        assert ghp.holdplace2cdlist() == expected

    tama_info = {"shinko": "9R以降中止", "ed_race_no": 9}
    hama_info = {"shinko": "-", "ed_race_no": 12}

    @pytest.mark.parametrize(
        "date, hp_name, expected",
        [(20110311, "多摩川", tama_info), (20110311, "浜名湖", hama_info)],
    )
